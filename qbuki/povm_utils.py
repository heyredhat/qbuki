# AUTOGENERATED! DO NOT EDIT! File to edit: ../nbs/povm_utils.ipynb.

# %% auto 0
__all__ = ['vn_povm', 'tighten', 'frame_povm', 'squish', 'complete', 'dilate', 'coarse_grain', 'sample_from_povm',
           'implement_povm', 'discriminator_povm', 'frame_quantumness', 'weighted_frame_potential',
           'minimum_real_frame_potential', 'minimum_complex_frame_potential', 'minimal_born']

# %% ../nbs/povm_utils.ipynb 4
import numpy as np
import scipy as sc

from .utils import *
#from qbuki.operators import *
...
#def coarse_grain(A, P):
#    return Operators(np.array([sum([P[j,i]*A[i] for j in range(P.shape[1])]) for i in range(len(A))]))

import cvxpy as cp

def minimal_born(P, p=2, return_problem=False):
    n = P.shape[0]
    u = np.ones(n)
    X = cp.Variable((n,n))
    problem = cp.Problem(cp.Minimize(cp.norm(np.eye(n) - X, p=p)), 
                                     [P @ X @ P == P, 
                                      u.T @ X == u.T])
    problem.solve()
    return X.value, problem if return_problem else X.value
