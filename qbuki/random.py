# AUTOGENERATED! DO NOT EDIT! File to edit: ../nbs/random.ipynb.

# %% auto 0
__all__ = ['rand_stochastic', 'rand_ginibre', 'rand_ket', 'rand_ket_hs', 'rand_herm', 'rand_dm', 'rand_unitary', 'rand_povm',
           'rand_effect', 'rand_ftf', 'rand_funtf', 'rand_kraus', 'rand_probs', 'rand_probs_table',
           'rand_quantum_probs_table', 'sample_convex_hull', 'random_born']
...
# %% ../nbs/random.ipynb 5
def rand_stochastic(m: int, # rows
                    n: int, # columns
                    axis: int = 0, # along
                    r: int = -1 # rank
                   ) -> np.ndarray:
    if r == -1:
        return np.random.dirichlet(m*(1,), size=n).T if axis == 0 else \
               np.random.dirichlet(n*(1,), size=m)
    else:
        return rand_stochastic(m, r, axis=axis) @ \
               rand_stochastic(r, n, axis=axis)
